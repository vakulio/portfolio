            ...
            <div className="px-0 md:px-10">
                ...
                <div className="flex space-x-2 my-2">
					{experienceOne.technologies.map(tech => (
						<img key={tech._id} className="h-10 w-10" src={urlFor(tech.image).url()} alt={tech.title} />
					))}
                </div>
                <p>{new Date(experienceOne.dateStarted).toDateString()} - {experienceOne.isCurrentWork ? "Present" : new Date(experienceOne.dateStarted).toDateString()}</p>
                <ul className="list-disc space-y-4 ml-5 text-lg h-70 overflow-y-scroll scrollbar-thin scrollbar-track-black scrollbar-thumb-[#F7AB0A]/80">
